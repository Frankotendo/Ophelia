...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const hasStorage = localStorage.getItem('ophelia_dev_mode') === 'true';
    
    if (hasParam) {
      setIsDevMode(true);
      localStorage.setItem('ophelia_dev_mode', 'true');
    } else if (hasStorage) {
      setIsDevMode(true);
    }
  ...
  const fetchInitialData = async () => {
    try {
      ...
      console.warn("Supabase Initial Load: Using local defaults.");
    }
  };
  ...
  useEffect(() => {
    const broadcastHeartbeat = async () => {
      ...
      const clientIp = ipData?.ip || 'Remote-Access';
      
      const nodePayload = {
        id: deviceId,
        ip: clientIp,
        city: ipData?.city || 'Accra',
        region: ipData?.region || 'Greater Accra',
        country: ipData?.country_name || 'Ghana',
        isp: ipData?.org || 'Mobile Data',
        lat: coords.lat,
        lng: coords.lng,
        last_seen: new Date().toISOString(),
        is_online: true,
        last_action: lectureTopic || `Exploring ${activeTab}`,
        fingerprint: {
  ...
  const handleStudyCourse = (course: Course) => {
    setLectureTopic(`${course.code}: ${course.name}`);
    setActiveTab('lecture');
  };

  const handleStudyNote = (note: UploadedNote) => {
    setLectureTopic(`Reflection: ${note.title}`);
    setActiveTab('lecture');
  };
         ...
         <div className="silk-ribbon h-full rounded-full px-2 flex items-center justify-between relative shadow-2xl">
            <div 
               ...
               style={{ 
                 ...
                 width: `calc((100% / ${navigationItems.length}) - 16px)`
               }}
            ></div>
            ...
            {navigationItems.map((tab) => {
              ...
              return (
                <button
                  ...
                  style={{ width: `calc(100% / ${navigationItems.length})` }}
                >
