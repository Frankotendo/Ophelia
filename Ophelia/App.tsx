...
const App: React.FC = () => {
  ...
  useEffect(() => {
    // Check URL and LocalStorage for dev mode
    const params = new URLSearchParams(window.location.search);
    const hasParam = params.get('dev') === 'true';
    const hasStorage = localStorage.getItem('ophelia_dev_mode') === 'true';
    
    // If the URL has ?dev=true, enable it and save to storage
    if (hasParam) {
      setIsDevMode(true);
      localStorage.setItem('ophelia_dev_mode', 'true');
    } else if (hasStorage) {
      // If it was previously enabled via param, keep it enabled
      setIsDevMode(true);
    }
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // Fix: Corrected property name from readiness_score to readinessScore to match ProgressState type
  ...
  useEffect(() => {
    ...
    broadcastHeartbeat();
    const interval = setInterval(broadcastHeartbeat, 30000);
